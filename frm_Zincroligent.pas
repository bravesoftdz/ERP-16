...
begin
  ...
  Try
    if tabla='1' then begin
      ...
      UserDBa:=Ini.ReadString(bd,'Usuario','root');
      PassDBa:=Ini.ReadString(bd,'Pass','danae');
    end else begin
      HostDBb:=Ini.ReadString('ConexionesDB',bd,'Localhost');
      PortDBb:=Ini.ReadInteger(bd,'Puerto',3306);
      UserDBb:=Ini.ReadString(bd,'Usuario','root');
      PassDBb:=Ini.ReadString(bd,'Pass','danae');
    end;
