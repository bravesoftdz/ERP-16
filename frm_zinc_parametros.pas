...
begin
  ...
  {if id='1' then begin
    ...
    PassDBa:=Ini.ReadString(Conexion,'Pass','danae');
  end else begin
    ...
    PassDBb:=Ini.ReadString(Conexion,'Pass','danae');
  end;}
  zConnection.HostName:=Ini.ReadString('ConexionesDB',Conexion,'Localhost');
  zConnection.Port:=Ini.ReadInteger(Conexion,'Puerto',3306);
  zConnection.User:=Ini.ReadString(Conexion,'Usuario','root');
  zConnection.Password:=Ini.ReadString(Conexion,'Pass','danae');
  zConnection.Connect;
  Ini.Free;
end;
end.
